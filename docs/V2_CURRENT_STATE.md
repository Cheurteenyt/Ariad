...
> **Authoritative snapshot of the current product state.** Updated R169B-STEP5 (2026-07-14) — recovery, evidence, metadata, CAS and documentation closure.
> R169A landed the generation-store foundation (path helpers, manifest V1 types, resolver, atomic JSON writer) and is merged. R169B-STEP2 added the publisher / CAS / GC primitives. R169B-STEP3 closed the 22 findings from the GPT 5.6 Pass 1 audit. R169B-STEP4 closed the 19 findings from the GPT 5.6 Pass 2 audit (immutability via copy/reflink, GC Model A). R169B-STEP5 (this revision) closes the 16 findings from the GPT 5.6 Pass 3 audit — token state machine with PublicationMutationPhase, CAS EEXIST race fix, metadata no-clobber, catalog rebuild after crash, final DB mode 0600. All five remain FOUNDATION / INACTIVE — the indexer and readers still use the legacy `<project>.db` path. `DATA-CARRY-01` (P1) remains open until R169E (after crash matrix + concurrency + performance + activation).
