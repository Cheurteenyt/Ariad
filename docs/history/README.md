# Historical documentation

> **Status:** Historical index
> **Audience:** Maintainers and auditors
> **Last verified:** 2026-07-24 R184 archive

Everything below this directory is non-canonical evidence. A historical file
remains valid for its pinned revision, but it must not be treated as current
product behavior unless the [documentation portal](../README.md) explicitly
cites it.

## Audits

- [Repository audit — 2026-07-15](audits/AUDIT_2026-07-15.md)
- [GLM 5.2 confirmed-error remediation](audits/GLM_5.2_CONFIRMED_ERRORS_REMEDIATION_9453537.md)
- [V1 C engine audit R65](audits/V1_AUDIT_R65.md)
- [Early V2 audit](audits/V2_AUDIT.md)

## Historical benchmarks

- [Performance benchmark R66](benchmarks/PERFORMANCE_BENCHMARK_R66.md)
- [V1/V2 benchmark R67](benchmarks/V1_V2_BENCHMARK_R67.md)
- [Rigorous benchmark R77](benchmarks/RIGOROUS_BENCHMARK_R77.md)
- [Rigorous benchmark R78](benchmarks/RIGOROUS_BENCHMARK_R78.md)

Later native-accounting evidence lives under
[`performance/reports/`](../performance/reports/).

## Roadmaps and architecture drafts

- [V2 roadmap archived at the 0.15.9 era](roadmaps/V2_ROADMAP.md)
- [Original V2 sidecar architecture](V2_ARCHITECTURE_ORIGINAL_SIDECAR_DESIGN.md)

## Round reports

- [R163 implementation report](round-reports/R163-coder.md)
- [R164 implementation report](round-reports/R164-coder.md)
- [R171 exact-lookup benchmark report](round-reports/R171_EXACT_LOOKUP_BENCHMARK.md)
- [R177 multi-hop caller correction](round-reports/R177_MULTI_HOP_CALLER_CORRECTION.md)
- [R178 fresh B/C multi-hop confirmation](round-reports/R178_FRESH_BC_MULTIHOP_CONFIRMATION.md)
- [R180 R179 forward-reference correction](round-reports/R180_R179_FORWARD_REFERENCE.md)
- [R181 structural cost root-cause correction](round-reports/R181_STRUCTURAL_COST_ROOT_CAUSE.md)
- [R182 maintenance consolidation](round-reports/R182_MAINTENANCE_CONSOLIDATION.md)
- [R183 Graph UI visual intelligence](round-reports/R183_GRAPH_UI_VISUAL_INTELLIGENCE.md)
- [R184 Ariad versus Graphify truth audit](round-reports/R184_ARIAD_GRAPHIFY_TRUTH_AUDIT.md)

## Changelog archive

The active package changelog keeps the current release window. Older entries
are preserved in [the changelog archive](changelog/CHANGELOG_0.12.0_TO_0.75.0.md).
